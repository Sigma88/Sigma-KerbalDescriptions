...
namespace SigmaKerbalDescriptions
{
    class RecoveryFix : MonoBehaviour
    {
        ...
        void Late()
        {
            if (!skip)
            {
                ...
                if (widget != null)
                {
                    Description.Update(new ListItemContainer(widget), null, widget.crew);
                    return;
                }
            }
        }
    ...
    class AssignmentFix : MonoBehaviour
    {
        ...
        void UpdateAssignments()
        {
            ...
            {
                Description.Update(new ListItemContainer(items[i]), items[i].GetTooltip(), items[i]?.GetCrewRef());
            }
        }
    }
